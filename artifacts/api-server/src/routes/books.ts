...
import { db, booksTable, usersTable, libraryTable, commentsTable } from "@workspace/db";
import {
  CreateBookBody,
  UpdateBookBody,
...
router.get("/books/trending", optionalAuth, async (req: any, res): Promise<void> => {
  ...
  const books = await Promise.all(trending.map((b) => buildBook(b, req.clerkUserId)));
...
router.patch("/books/:bookId", requireAuth, async (req: any, res): Promise<void> => {
  const raw = Array.isArray(req.params.bookId) ? req.params.bookId[0] : req.params.bookId;
  const bookId = parseInt(raw, 10);
  ...
  if (parsed.data.coverObjectPath != null) updates.coverObjectPath = parsed.data.coverObjectPath;
...
router.delete("/books/:bookId", requireAuth, async (req: any, res): Promise<void> => {
  const raw = Array.isArray(req.params.bookId) ? req.params.bookId[0] : req.params.bookId;
  const bookId = parseInt(raw, 10);
...
router.get("/users/:userId/books", optionalAuth, async (req: any, res): Promise<void> => {
  ...
  if (showPrivate) {
    ...
    userBooks = await db
      .select()
      .from(booksTable)
      .where(and(eq(booksTable.authorClerkId, raw), eq(booksTable.isPublic, true)))
      .orderBy(desc(booksTable.createdAt));
  }

  const books = await Promise.all(userBooks.map((b) => buildBook(b, req.clerkUserId)));
  res.json({ books, total: books.length });
});
